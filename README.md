...
| `Ctrl` + `v` | Normal / Insert | **OSのクリップボードから貼り付け** (テキストおよび画像に対応) |
...
| `:img` |  | **画像挿入** | 画像ファイルを選択し、プロジェクト内の `attachments/` に保存して挿入します。 |
...
| `:design` |  | **設定画面** | フォントサイズやテーマカラーを変更するデザインモードを開きます。 |
...
* **ドラッグ&ドロップ**: 画像ファイルをエディタにドロップすると、自動的にプロジェクト内の `attachments` フォルダへコピーされ、Markdown形式 `![image](path)` で挿入されます。

## 設定・カスタマイズ

フォントサイズ、行間、テーマカラーなどの外観設定は、`:design` コマンドで変更できます。
プレビューを見ながらスライダーで直感的に調整可能です。

![Design Mode](screenshot_design.png)
### ディレクトリ構成
ノートや設定ファイルは、実行ファイルと同じディレクトリ（または指定のVault）に以下のように整理されます。バックアップの際はこのフォルダごとコピーしてください。

```text
My_Notebook/
 ├── notes/           # Markdownファイル (日々の記録)
 ├── attachments/     # 画像などのメディアファイル
 └── .vsim/           # 設定ファイル (settings.json等)
...
* **鉄壁のIME制御**: `Esc` でInsertモードを抜けるとIMEが自動でOFFになり、Normalモードでの誤入力も強力にブロックされます。
